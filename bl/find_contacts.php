...
if (isset($_SESSION["user_info"])) {
    ...
    $list = $contactDAL->getContactsByName($user->getUserID(), $name, 0);
    // Nếu có dữ liệu thì hiển thị bảng ra
    echo "<h5>Search result(s) for " . "\"" . $name . "\"" .  "</h5>";
    if (count($list) > 0) {
        // Tiêu đề cột
                    ...
                    <td>
                        ...
                        <button type='button' class='btn btn-danger' data-confirm='Are you sure you want to delete this contact?' 
                            ...
                            if (confirm(this.getAttribute('data-confirm'))) { 
                                window.location.href='./bl/delete_customer.php?contact_id=" . $contact->getContactId() . "'; 
                            }
                            \">
                            <i class=\"fa-solid fa-trash fa-lg\"></i>
                        </button>
                        <button type='button' class='btn btn-danger' data-confirm='Are you sure you want to block this contact?' 
                            onclick=\"
                            if (confirm(this.getAttribute('data-confirm'))) { 
                                window.location.href='../bl/block_contact.php?contact_id=" . $contact->getContactID() . "'; 
                            }
                            \">
                            <i class=\"fa-solid fa-ban fa-lg\" style=\"color: #ffffff;\"></i>
                        </button>
                    </td>
                </tr>";
            $i++;
